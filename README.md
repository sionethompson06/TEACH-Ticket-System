...
**Phase 5 — Core Ticket Foundation.**

Phase 0 established the project's written foundation; Phase 1 added a minimal, feature-empty Next.js/TypeScript application shell; Phase 2 added a provider-neutral PostgreSQL schema and canonical reference data; Phase 3 added Google Workspace sign-in restricted to verified `@teachps.org` accounts, with first-login provisioning of a fixed, nonprivileged **Requester** profile (external OAuth configuration and live sign-in acceptance remain a deferred, separately approved operational step); Phase 4 added a minimal Requester/Department-Agent/System-Administrator access-control model. Phase 5 adds the simple database and server-service foundation for a basic help-desk ticket: seeded IT/Facilities categories, a `tickets` table with a database-generated human-friendly number (`TKT-000001`), a small documented status/priority set, append-only public comments, a narrow ticket-activity log, and a server-only ticket service enforcing the Phase 4 authorization model. **No user interface for tickets exists yet — Phase 6 will build the requester-facing experience — and no real user, ticket, or comment data has been created.** See [`docs/DATABASE.md`](docs/DATABASE.md) for the ticket data model, [`docs/AUTHENTICATION.md`](docs/AUTHENTICATION.md) for the authentication foundation, and [`docs/PHASE_PLAN.md`](docs/PHASE_PLAN.md) for what comes next.
