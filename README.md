...
**Phase 6 — Requester Experience.**

Phase 0 established the project's written foundation; Phase 1 added a minimal, feature-empty Next.js/TypeScript application shell; Phase 2 added a provider-neutral PostgreSQL schema and canonical reference data; Phase 3 added Google Workspace sign-in restricted to verified `@teachps.org` accounts, with first-login provisioning of a fixed, nonprivileged **Requester** profile (external OAuth configuration and live sign-in acceptance remain a deferred, separately approved operational step); Phase 4 added a minimal Requester/Department-Agent/System-Administrator access-control model; Phase 5 added the database and server-service foundation for a basic help-desk ticket (seeded IT/Facilities categories, a `tickets` table with a database-generated human-friendly number such as `TKT-000001`, a documented status/priority set, append-only public comments, a ticket-activity log, and a server-only ticket service enforcing the Phase 4 authorization model). Phase 6 adds the first user-facing experience: a signed-in staff member can now **Request Help**, see **My Requests**, open a request, and **Send Message** to continue the conversation with the support team. All requester pages require a signed-in session, and every action is re-checked against the Phase 4/5 authorization rules server-side. **No department-agent workspace exists yet — Phase 7 will build the IT/Facilities work-queue and assignment experience — and Google Workspace live OAuth acceptance remains a deferred, separately approved operational step.** See [`docs/DATABASE.md`](docs/DATABASE.md) for the ticket data model, [`docs/AUTHENTICATION.md`](docs/AUTHENTICATION.md) for the authentication foundation, and [`docs/PHASE_PLAN.md`](docs/PHASE_PLAN.md) for what comes next.
