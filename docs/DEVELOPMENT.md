...
`PUBLIC_TICKET_INTAKE=true` temporarily enables unauthenticated ticket submission at `/requests/new` for use while Google OAuth is unavailable — see [`docs/AUTHENTICATION.md`](AUTHENTICATION.md) for the full security model and [`docs/DEPLOYMENT.md`](DEPLOYMENT.md) for the setup sequence. Locally, to exercise it:

```bash
PUBLIC_TICKET_INTAKE=true PUBLIC_INTAKE_RATE_LIMIT_SECRET="$(openssl rand -base64 32)" npm run dev
```

- Only the exact literal value `"true"` enables it; any other value (or its absence, the default) leaves `/requests/new` requiring an active authenticated actor exactly as before.
- `PUBLIC_INTAKE_RATE_LIMIT_SECRET` must be at least 32 characters whenever the flag is on — a separate secret from `BETTER_AUTH_SECRET`, never shared with it.
- An active, authenticated actor always sees the normal authenticated form regardless of this flag; only a signed-out (or not-yet-configured-auth) visitor sees the public form.
- `/requests`, `/requests/[ticketNumber]`, `/support`, `/admin`, and `/account` are completely unaffected by this flag.
