...
`PUBLIC_TICKET_INTAKE=true` (`src/public-intake/env.ts`) enables a **separate, temporary path** for submitting a new IT or Facilities request without signing in at all — intended only for the gap while Google OAuth credentials aren't yet configured in a given environment. It is not an authentication mode, and it changes nothing about the sign-in flow described above:

- **Fails closed like everything else here.** Only the exact literal value `"true"` enables it; anything else (including the variable being absent) leaves it off, and every route behaves exactly as it does today.
- **Never weakens `requireActiveActor` or `resolveActor`.** An anonymous visitor is never turned into a `ResolvedActor`, active or otherwise — public submission uses an entirely separate code path (`src/tickets/public-intake-service.ts`) that has no concept of an actor and never calls `authorize()`.
- **Only `/requests/new` and its confirmation page (`/requests/new/submitted`) change behavior.** `/requests`, `/requests/[ticketNumber]`, `/support`, `/admin`, and `/account` are completely untouched and still require an active, authenticated actor exactly as before — this flag never makes any ticket list, ticket detail, comment, support, or admin functionality public.
- **The confirmation page performs no database query.** It only format-validates the `ticket` query-parameter before displaying it, so a ticket number — real, guessed, or fabricated — can never be used to look up or access anything (see [`DECISION_LOG.md`](DECISION_LOG.md) D-085).
- **The submitting requester is never an application user.** Every publicly submitted ticket's `requester_id` points at one reserved, fixed, **inactive** system user (`public-intake@teach-ticket.invalid`, seeded by `seedReferenceData()` — see [`DATABASE.md`](DATABASE.md)), which can never resolve as an active actor and has no Better Auth account, session, or credential of any kind. The requester's actual name and email are stored only as ticket-level snapshots (`tickets.public_requester_name`/`public_requester_email`) for support staff to contact later — never shown on the public confirmation page, never placed in a URL, and never usable to sign in.
- **Abuse protection is layered and fails closed.** A visually hidden honeypot field silently discards a completed submission (no ticket created, same generic confirmation page). A durable, concurrency-safe, database-backed rate limiter (`public_intake_rate_limits`) allows roughly five submissions per hour per pseudonymous fingerprint — an HMAC of a resolved client IP under a **separate** secret, `PUBLIC_INTAKE_RATE_LIMIT_SECRET` (required to be at least 32 characters whenever public intake is enabled). If the rate-limit table can't be read or written at all, the submission is denied, never silently allowed through. See [`DECISION_LOG.md`](DECISION_LOG.md) D-081–D-083 for the full rationale.
- **Independent of `AUTH_ACCESS_MODE`.** Public intake mode does not require `AUTH_ACCESS_MODE` to be set at all, and setting it has no effect on public intake. A deployment can run public intake alone (no Google OAuth configured yet), Google OAuth alone (public intake off, today's behavior), or — during a transition — both together, in which case an already-signed-in active actor always sees the normal authenticated form and everyone else sees the public one.
- **Never implies support or admin access works without Google OAuth.** Enabling this flag only ever adds a way to submit a ticket; the home page, `/requests/new`, and `npm run readiness:check` all continue to state that staff sign-in, Support Queue, and Administration remain unavailable until Google OAuth is configured.
...
| `PUBLIC_TICKET_INTAKE`            | (Phase 9B) `true` temporarily enables unauthenticated ticket submission at `/requests/new`. Any other value (or absence) disables it, independent of `AUTH_ACCESS_MODE`.                           |
| `PUBLIC_INTAKE_RATE_LIMIT_SECRET` | (Phase 9B) Required only when `PUBLIC_TICKET_INTAKE=true`. A separate secret (never reuse `BETTER_AUTH_SECRET`) of at least 32 characters, used only to fingerprint submissions for rate limiting. |
...
- **Public ticket intake** (Phase 9B — `src/public-intake/env.test.ts`, `src/public-intake/rate-limit.test.ts`, `src/db/public-intake.test.ts`, `src/app/requests/new/page.test.tsx`, `src/app/requests/new/public-request-form.test.tsx`, `src/app/requests/new/public-actions.test.ts`, `src/app/requests/new/submitted/page.test.tsx`, `src/app/app-nav.test.tsx`) — the flag's fail-closed parsing; the reserved Public Intake user's existence and inactivity; anonymous access to the form only when enabled, with authenticated behavior unchanged either way; valid/invalid public submissions; honeypot and rate-limit behavior (including concurrency-safety and fail-closed configuration); the confirmation page's no-database-lookup guarantee; and that a ticket number alone grants no access.
