...
This document covers the Phase 2 database foundation: the PostgreSQL schema, migration workflow, and canonical reference data. Phase 3 added the authentication tables described briefly below (see [`AUTHENTICATION.md`](AUTHENTICATION.md) for the full authentication design); Phase 4 added a minimal access-control model (departments, department agents, and a single system-administrator flag); Phase 5 added the core ticket data model and server-only ticket service; Phase 9A added one small pilot-invitation table supporting a controlled, invite-only access mode alongside the original strict Workspace-domain mode; Phase 9B added a temporary, unauthenticated public ticket-intake path (a submission-source column and public-requester snapshot on `tickets`, a reserved system user, and a rate-limit table). No ticket UI, SLA calculation, department-manager role, or confidential-access grant exists yet (see [`PHASE_PLAN.md`](PHASE_PLAN.md)).
...
Four additional tables — `user`, `account`, `session`, and `verification` — support Google sign-in and first-login provisioning. They are described fully in [`AUTHENTICATION.md`](AUTHENTICATION.md), including the `CHECK` constraints that fix every user to the canonical organization and the Requester role, forbid persisting any OAuth token or password, and forbid any provider other than `google`. Phase 3 also added two plain columns to `user` used only starting in Phase 4: `is_active` (boolean, default `true`) and `is_system_administrator` (boolean, default `false`, described below).

**Phase 9A note:** `user`'s email `CHECK` constraint was originally `@teachps.org`-exact; it is now domain-agnostic (verified, lowercased, well-formed email of any domain) since the schema must support both the strict `workspace` access mode and the `invite_only` pilot mode, and a database constraint cannot vary by runtime configuration. Domain eligibility is enforced entirely at the application layer now (`src/auth/google-identity-policy.ts`). See [`AUTHENTICATION.md`](AUTHENTICATION.md) for the full detail.

## Phase 9A Table — Pilot Invitations

One additional table, `auth_invitations`, supports the `invite_only` access mode (see [`AUTHENTICATION.md`](AUTHENTICATION.md) for the full design). A row means "this email is, or was, invited" — nothing more; it is never a role, and accepting one only ever allows a first sign-in to provision a plain requester.

- UUID primary key, `organization_id` (fixed to the canonical organization by a `CHECK` constraint, like `user`), normalized `email`, `status` (`pending` | `accepted` | `revoked`, `CHECK`-constrained), `created_source` (`cli` | `admin_ui`, `CHECK`-constrained), an optional `created_by_user_id`, `created_at`, an optional `accepted_by_user_id`/`accepted_at`, an optional `revoked_by_user_id`/`revoked_at`.
- A `CHECK` constraint enforces that the status and acceptance/revocation columns always agree — a `pending` row has none of the acceptance/revocation columns set, an `accepted` row has exactly the acceptance columns set, a `revoked` row has exactly the revocation columns set.
- A partial unique index (`auth_invitations_pending_email_unique_idx`, on `(organization_id, email)` `WHERE status = 'pending'`) allows at most one _current_ invitation per email at a time, while never restricting how many historical (accepted/revoked) rows exist for that email — a fresh invitation can always be created for a previously revoked address.
- Composite foreign keys tie each of `created_by_user_id`, `accepted_by_user_id`, and `revoked_by_user_id` (when set) to `user(id, organization_id)` — the same organization-scoping pattern used by `tickets` and `department_memberships`.
- No invitation is seeded. History (accepted/revoked rows) is never destructively deleted — only `INSERT` (create) and `UPDATE` (accept/revoke) are ever used.

Migration `drizzle/0005_phase9a_invite_only_access.sql` creates this table and replaces `user`'s email `CHECK` constraint (above). It does not modify migrations `0000`–`0004` in any way.
...
- **Submission source (Phase 9B):** `submission_source` (`'authenticated'` | `'public'`, `CHECK`-enforced, defaulting `'authenticated'`) distinguishes an ordinary authenticated submission from one made through the temporary public intake path — see [Phase 9B Additions](#phase-9b-additions--temporary-public-ticket-intake) below. Every ticket created before this column existed reads `'authenticated'`.
- **Public requester snapshot (Phase 9B):** nullable `public_requester_name`/`public_requester_email` capture the submitter's name/email only for a public ticket, for support staff to contact later — never on an authenticated ticket. A `CHECK` constraint ties their presence exactly to `submission_source`: both set (non-blank, within length, and a well-formed lowercased email) when `'public'`, both `NULL` when `'authenticated'`. Never displayed on the public confirmation page or placed in a URL.
...
## Phase 9B Additions — Temporary Public Ticket Intake

Migration `0006` adds one table and three columns supporting a temporary, unauthenticated ticket-submission path (`PUBLIC_TICKET_INTAKE=true` — see [`AUTHENTICATION.md`](AUTHENTICATION.md) for the full security model). It does not modify migrations `0000`–`0005` in any way.

### Reserved "Public Intake" User

Rather than making `tickets.requester_id` nullable, every publicly submitted ticket is attributed to one reserved, fixed-id `user` row (`REFERENCE_PUBLIC_INTAKE_USER` in [`src/db/reference-data.ts`](../src/db/reference-data.ts)):

- Email `public-intake@teach-ticket.invalid` — the reserved `.invalid` TLD (RFC 2606) guarantees it is never a real, deliverable address.
- `is_active = false` — permanently, so `resolveActor()` always returns `"inactive"` for it and it can never pass `requireActiveActor` or any `authorize()` check, even if a session somehow referenced its id.
- No `account`, `session`, or `verification` row exists for it, and none ever will — it has no Better Auth credential of any kind and cannot sign in.
- Inserted idempotently by `seedReferenceData()` (not by migration `0006` itself — see [`DECISION_LOG.md`](DECISION_LOG.md) D-078 for why: its `organization_id` foreign key requires the organization row that only the seed creates), so a fresh clone always has it after the standard `db:migrate` + `db:seed` setup.
- Used only as `tickets.requester_id` and `ticket_activity.acting_user_id` for public submissions — never returned to a public caller, never rendered on the public confirmation page.

### `public_intake_rate_limits`

UUID-free, keyed directly by `fingerprint` (text primary key), `window_start`, `count`, `updated_at`.

- `fingerprint` is an HMAC-SHA256 hex digest of a resolved client IP address, keyed by `PUBLIC_INTAKE_RATE_LIMIT_SECRET` — the raw IP address is never stored anywhere, including here.
- One fixed one-hour window per fingerprint (`window_start`, truncated to the hour); `count` increments within the current window and resets on a new one.
- Every check-and-increment is a single atomic `INSERT ... ON CONFLICT (fingerprint) DO UPDATE` statement (`src/public-intake/rate-limit.ts`) — Postgres's own row-level locking during the conflict update keeps this correct under concurrent writes from multiple Vercel Function instances, with no separate select-then-update race window.
- If this table can't be read or written at all, the submission is denied (fail closed) — never silently allowed through.

### Public Intake Service

`src/tickets/public-intake-service.ts` exposes one function, `createPublicTicket`, entirely separate from `ticket-service.ts`'s `createTicket`:

- Takes no `ResolvedActor` and never calls `authorize()` — there is no authenticated actor for a public submission to check.
- Hardcodes the organization to the canonical TEACH organization and the requester to the reserved Public Intake user — its input type has no field for a caller to name either one.
- Validates the requester's name/email (normalized, length-limited, well-formed) and every existing field (department/category/location membership, subject/description) exactly as strictly as the authenticated path.
...
This runs `src/db/database-foundation.test.ts`, `src/db/tickets.test.ts`, `src/db/admin.test.ts`, `src/db/admin-bootstrap.test.ts`, `src/db/invitations.test.ts`, `src/db/auth-provisioning.test.ts`, and `src/db/public-intake.test.ts` against a completely fresh, in-memory PGlite instance: it applies the committed migrations, confirms only the fifteen approved tables exist (the three Phase 2 reference tables, the four Phase 3 authentication tables, the two Phase 4 access-control tables, the four Phase 5 ticket tables, the one Phase 9A pilot-invitation table, and the one Phase 9B rate-limit table — Phases 6, 7, and 8 add no new tables), runs the seed, verifies exact record counts and relationships (including that IT and Facilities are the only two departments, with exactly their 7 and 8 confirmed categories), reseeds to confirm idempotency, and confirms the database itself rejects duplicate codes, invalid foreign keys, invalid location-type structural combinations, duplicate department memberships, cross-organization memberships, a category/department mismatch, blank ticket/comment content, a non-lowercased or malformed user email (any domain), and every remaining authentication invariant described in [`AUTHENTICATION.md`](AUTHENTICATION.md). It also confirms the seed creates no ordinary/active user, invitation, department membership, administrator, ticket, comment, or activity record — the sole exception being the one reserved, inactive Phase 9B Public Intake system user — and exercises the full `ticket-service.ts` authorization surface (requester/department-agent/administrator access, cross-organization denial, status/priority/assignment rules, activity-history writes, and that a rejected mutation leaves no activity record behind), including the Phase 7 closed-ticket rule (a closed ticket rejects a new comment, a reassignment, and a priority change alike). It also exercises the Phase 6 read-only query layer (`src/tickets/ticket-queries.ts`) and the Phase 7 support query layer (`src/tickets/support-queries.ts`) — see the earlier phase notes below for exactly what each proves. It also exercises the Phase 8 administration module (`src/admin/`): that `listOrganizationUsers` is denied to anyone but an active system administrator, scopes strictly to the actor's own organization, sorts by name, supports a case-insensitive name/email search, and never selects a field beyond display name, email, active status, administrator status, and department codes; that `setDepartmentMembership` adds/removes access idempotently and rejects an invalid department code; that `setUserActive` deactivates/reactivates a target user, refuses to let an administrator deactivate themselves, and leaves that user's tickets and comments untouched; that a deactivated user's next `resolveActor()` call immediately returns `"inactive"`, failing both requester and support authorization; and that `setSystemAdministrator` grants/removes access while refusing to let an administrator remove their own. It also exercises the Phase 9A invitation lifecycle and provisioning gate: the guarded CLI's dry-run/apply/idempotency behavior; the admin-authorized Pilot Invitations service's authorization, organization scoping, and duplicate/non-pending rejection; the real `databaseHooks.user.create` before/after hooks denying a noninvited or revoked address, accepting an invited personal-Gmail or other-Workspace-domain address, marking an invitation accepted, and safely no-oping on a retried accept; and both access modes' administrator-bootstrap eligibility rules (exact-domain in workspace mode; linked-Google-account-plus-accepted-invitation in invite-only mode, no domain required). It also exercises the Phase 9B public-intake surface: the reserved Public Intake user's existence and permanent inactivity; `createPublicTicket` creating a ticket for the fixed organization and reserved requester with a correctly normalized public-requester snapshot, ignoring any organization/requester id supplied by the caller, and rejecting an invalid department/category/location/name/email; that a ticket number alone grants no access through the existing authorization-scoped query layer; and the rate limiter's concurrency-safety, per-fingerprint independence, and guarantee that no raw IP address is ever written to the rate-limit table. No external credentials or real database are involved — this is how a fresh clone proves the schema and seed work correctly, in CI and locally alike.
...
Beyond the fixed Requester role (Phase 3), the minimal department/agent/administrator model (Phase 4), the core ticket foundation (Phase 5), the requester-facing interface (Phase 6), the IT/Facilities support workspace (Phase 7), the minimal administration page (Phase 8), the pilot-invitation table (Phase 9A), and the temporary public intake additions (Phase 9B), the database contains **no department-manager role, no campus/principal grant, no confidential-access grant, no expiring permission grant, no SLA/business-calendar calculation, no internal/private-note visibility rule, and no email allowlist**. Invitation is a database row, never a role — accepting one only ever provisions the same fixed Requester profile every other sign-in does. Phases 6, 7, and 8 added no new tables or columns — all three read and write the existing Phase 3–5 tables only, through authorization-scoped queries and services. Public intake never creates an application user, session, or elevated role either — every public ticket is attributed to the one reserved, permanently inactive system user described above. A future phase will add requester-visible internal notes and a follower model on top of what exists here.
