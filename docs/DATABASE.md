...
This runs `src/db/database-foundation.test.ts` and `src/db/tickets.test.ts` against a completely fresh, in-memory PGlite instance: it applies the committed migrations, confirms only the thirteen approved tables exist (the three Phase 2 reference tables, the four Phase 3 authentication tables, the two Phase 4 access-control tables, and the four Phase 5 ticket tables — Phases 6 and 7 add no new tables), runs the seed, verifies exact record counts and relationships (including that IT and Facilities are the only two departments, with exactly their 7 and 8 confirmed categories), reseeds to confirm idempotency, and confirms the database itself rejects duplicate codes, invalid foreign keys, invalid location-type structural combinations, duplicate department memberships, cross-organization memberships, a category/department mismatch, blank ticket/comment content, and every authentication invariant described in [`AUTHENTICATION.md`](AUTHENTICATION.md). It also confirms the seed creates no user, department membership, administrator, ticket, comment, or activity record, and exercises the full `ticket-service.ts` authorization surface (requester/department-agent/administrator access, cross-organization denial, status/priority/assignment rules, activity-history writes, and that a rejected mutation leaves no activity record behind), including the Phase 7 closed-ticket rule (a closed ticket rejects a new comment, a reassignment, and a priority change alike). It also exercises the Phase 6 read-only query layer (`src/tickets/ticket-queries.ts`): that `listMyTickets` returns only the current requester's own tickets (even when they are also a department agent) sorted by most recently updated; that `getTicketDetailByNumber` returns `null` identically for a malformed ticket number, a nonexistent one, and one the actor is not authorized to view; that `listTicketComments` returns comments in chronological order with correct requester/support-team attribution and `null` for an unauthorized or nonexistent ticket; and that `loadTicketFormOptions` returns only active departments, categories, and locations. It also exercises the Phase 7 support query layer (`src/tickets/support-queries.ts`): that `listSupportFilterOptions` offers only a department agent's own memberships (or every department for a system administrator); that `listSupportQueueTickets` scopes to the actor's authorized departments, shows only active tickets by default, orders by priority then age, honors valid filters, and silently ignores invalid or unauthorized ones; that `getSupportTicketDetailByNumber` is gated by the same `manage_ticket` rule the ticket service uses, so an ordinary requester (even the ticket's own requester) is denied identically to a nonexistent ticket; that `listActiveDepartmentAgents` returns only active agents of the ticket's own department; and that `listTicketActivity` renders friendly, chronological activity text with no raw metadata. No external credentials or real database are involved — this is how a fresh clone proves the schema and seed work correctly, in CI and locally alike.
...
Beyond the fixed Requester role (Phase 3), the minimal department/agent/administrator model (Phase 4), the core ticket foundation (Phase 5), the requester-facing interface (Phase 6), and the IT/Facilities support workspace (Phase 7), the database contains **no department-manager role, no campus/principal grant, no confidential-access grant, no expiring permission grant, no SLA/business-calendar calculation, and no internal/private-note visibility rule**. Phases 6 and 7 added no new tables or columns — both read and write the existing Phase 5 tables only, through authorization-scoped queries and the same `ticket-service.ts` mutations. Phase 8 will add requester-visible internal notes and a follower model on top of what exists here.
