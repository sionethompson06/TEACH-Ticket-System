...
const { sendMessageAction } = vi.hoisted(() => ({
...
describe("SendMessageForm", () => {
  beforeEach(() => {
    ...
    sendMessageAction.mockImplementation(async () => ({
      status: "idle",
      body: "",
    }));
  });
  ...
  it("shows a pending state while sending", async () => {
    ...
    sendMessageAction.mockImplementationOnce(
      () =>
        new Promise((resolve) => {
  ...
  it("clears the message field after a successful send", async () => {
    sendMessageAction.mockResolvedValueOnce({ status: "success", body: "" });

    render(<SendMessageForm ticketId="ticket-1" ticketNumber="TKT-000001" />);
    const textarea = screen.getByLabelText(/message/i) as HTMLTextAreaElement;
    fireEvent.change(textarea, { target: { value: "Any update?" } });
    fireEvent.click(screen.getByRole("button", { name: /send message/i }));
